...
from .zoo import MIMOVL_OPERATIONS, MimoVLModel
...
def load_model(model_name, model_path, **kwargs):
    ...
    logger.info(f"Loading MimiVLModel model from {model_path}")

    # Create and return the model - operations specified at apply time
    return MimoVLModel(model_path=model_path, **kwargs)
...
def resolve_input(model_name, ctx):
    ...
    inputs.enum(
        "operation",
        values=list(MIMOVL_OPERATIONS.keys()),
        default=None,
        required=True,
        label="Operation",
        description="Type of task to perform with MiMo-VL model",
